...
router.get('/', auth, async (req, res) => {
  try {
    let jobs = [];
    if (req.user.role === 'admin') jobs = await db.jobs.find({});
    ...
    } else jobs = await db.jobs.find({ client_id: req.user.id });

    jobs = await Promise.all(jobs.map(async j => {
      const client = await db.users.findOne({ _id: j.client_id });
      let workerName = null;
      if (j.worker_id) {
        const w = await db.workers.findOne({ _id: j.worker_id });
        const wu = w ? await db.users.findOne({ _id: w.user_id }) : null;
        workerName = wu?.name;
      }
      // Adauga review daca exista
      const review = await db.reviews.findOne({ job_id: j._id });
      return { ...j, client_name: client?.name, worker_name: workerName, review: review || null };
    }));
...
// Meserias marcheaza job ca finalizat
router.patch('/:id/complete', auth, requireRole('meserias'), async (req, res) => {
  try {
    const job = await db.jobs.findOne({ _id: req.params.id });
    if (!job) return res.status(404).json({ error: 'Job negăsit' });
    await db.jobs.update({ _id: req.params.id }, { $set: { status: 'completed', completed_at: new Date().toISOString() } });
    res.json({ message: 'Job finalizat! Clientul poate acum lăsa un review.' });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
