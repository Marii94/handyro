...
router.patch('/workers/:workerId/price', async (req, res) => {
  try {
    const price = Number(req.body.price_per_job);
    if (!price || price < 1) return res.status(400).json({ error: 'Preț invalid' });
    const result = await db.workers.update({ _id: req.params.workerId }, { $set: { price_per_job: price } }, { returnUpdatedDocs: true });
    if (!result) return res.status(404).json({ error: 'Meșterul nu există' });
