...
async function seedIfEmpty() {
  ...
  const admin = await User.create({ name: 'Admin HandyRO', email: 'admin@handyro.ro', password: hash('admin123'), role: 'admin', status: 'active', email_verified: true });
  const ionut = await User.create({ name: 'Ionuț Șerban', email: 'ionut@handyro.ro', password: hash('ionut123'), role: 'meserias', status: 'active', email_verified: true });
  const mihai = await User.create({ name: 'Mihai Ionescu', email: 'mihai@handyro.ro', password: hash('mihai123'), role: 'meserias', status: 'active', email_verified: true });
  const maria = await User.create({ name: 'Maria Popescu', email: 'maria@handyro.ro', password: hash('maria123'), role: 'client', status: 'active', email_verified: true });
  const dan   = await User.create({ name: 'Dan Radu', email: 'dan@handyro.ro', password: hash('dan123'), role: 'client', status: 'active', email_verified: true });
